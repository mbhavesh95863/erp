	...
	refresh: function(frm) {
		...
		if(frm.doc.status == "Saved") {
			...
			frm.add_custom_button(__("Duplicate Template"), function() {
				frappe.call({
					method:'erpnext.salesapi.makeTemplate',
					args: {
						mso_id: frm.doc.name,
						shipping_date: frm.doc.shipping_date,
						delivery_date: frm.doc.delivery_date,
						pallet_weight: frm.doc.pallet_weight,
						usd: frm.doc.usd,
					},
					callback: function(r) {
						console.log(r.message);
						frappe.ui.toolbar.clear_cache();
						frappe.set_route("Form", "Master Sales Order Template", r.message);
						//location.reload();
					}
				});
		...
		console.log(frm.doc.template_mso);
		//if(frm.doc.template_mso) { frm.doc.name = frm.doc.template_mso;  frm.doc.status = "Draft"; }
...
table = table + '<td data-row="0" data-col="'+dataCol1 +'"><a href="javascript:;" class="delCol" onClick="salesTable_DelCol('+ dataCol1 +')">'+show_x+'</a></td>';
...
table = table + '<tr class="item-row" data-row="'+dataRow+'"><td data-row="'+dataRow+'" data-col="0"></td><td data-row="'+dataRow+'" data-col="1"></td><td data-row="'+dataRow+'" data-col="2"></td><td data-row="'+dataRow+'" data-col="3"></td>'+tlCols+'<td data-row="'+dataRow+'" data-col="'+ ttlcol + '"><span class="qtytotal_total"></span></td><td data-row="'+dataRow+'" data-col="'+ (ttlcol+1) + '"><span class="ppktotal_total"></span></td></tr>';
	...
	$wrapper.html('<div style="overflow-x:auto;"><table id="salesTable"  border="1" class="display nowrap dataTable dtr-inline">'+table+'</table><script type="text/javascript">$("#salesTable").on("click", ".isupplier", function() { var idd = $(this).attr("id"); autocompletesupplier1(idd); }); $("#salesTable").on("click", ".address_title", function() { var idd = $(this).attr("id"); autocompleteaddress1(idd); }); $("#salesTable").on("change", "input", function() { $(this).updatedata(); }); $("#salesTable").on("click", ".sitems", function() { var row = $(this).parents(".item-row"); var idd = row.find(".sitems").attr("id"); autocomplete1(idd); }); $("#salesTable").find(".sitems").each( function(){ $(this).updatedata(); });</script></div>');
				...
				if(frm.doc.status == "Saved") {
					$("#salesTable input").prop("disabled", true);
		...
		if(frm.doc.status == "Draft") {
				...
				$("#salesTable").find(".sitems").each( function()
				{
					...
					if(row.find('.item_code').val() != '') 
					{
						...
						for(var i = 4; i < (colCount-2); i++) 
						{
							...
							var listitm = {'name': iname, 'qty': iqty, 'price': iprice, 'weight': row.find('.weight').val(), 'weight_per_unit': row.find('.weight').val(), 'gross_weight': row.find('.gross_weight').val(), 'uom': row.find('.uom').val(), 'description': row.find('.idesc').val(), 'purchase_pallets': row.find('.purchase_pallets').val(), 'item_code': row.find('.item_code').val(), 'ppk_calculation': row.find('.ppk_calculation').val(), 'item_name': row.find('.idesc').val(),'received_qty' : rqty, 'customer_name': $('#isupplier'+i).val(), 'customer': $('#isupplier'+i).val(), 'default_currency': sidcell.find('.default_currency').val(), 'row_number': vindex , 'col_number': col_number,'mso_notes':mso_notes, 'po_no':po_no, 'address_title':address_title, 'address':address};
				...
				$("#salesTable").find(".sitems").each( function()
				{
					...
					if(row.find('.item_code').val() != '') 
					{
						...
						for(var i = 4; i < (colCount-2); i++) 
						{
							...
							if(isNaN(iqty)) { row.find('.iqty'+i).val(0);  }
							if(isNaN(rqty)) { row.find('.rqty'+i).val(0);  }
							if(isNaN(iprice)) { row.find('.iprice'+i).val(0.0); iprice = 0;  }
			...
			var TableArray = [];
								...
								for (var i = 0; i < MasterPOItems.master_sales_order_item.length; i++) 
								{
									...
									var itmcd1 = { 'customer_name': dta.customer_name, 'customer': dta.customer , 'default_price_list': dta.default_price_list ,'delivery_date': MasterPOItems.delivery_date ,'shipping_date': MasterPOItems.shipping_date , 'transaction_date': MasterPOItems.shipping_date , 'cad': MasterPOItems.cad,  'usd': MasterPOItems.usd,  'conversion_rate': conversion_rate, 'plc_conversion_rate': plc_conversion_rate, 'row_number': dta.row_number, 'col_number':  dta.col_number, 'total_boxes': dta.total_boxes, 'total_pallet': dta.total_pallet, 'total_net_weight': dta.total_net_weight, 'total_gross_weight_lbs': dta.total_gross_weight_lbs, 'total_weight_kg': dta.total_weight_kg,'mso_notes':dta.mso_notes, 'po_no':dta.po_no, 'address_title':dta.address_title, 'address':dta.address, items: [] };				
									var splcd = { 'item_name': dta.description ,  'item_code': dta.item_code,  'ppk_calculation': dta.ppk_calculation, 'weight': dta.weight, 'weight_per_unit': dta.weight_per_unit, 'weight_lbs': dta.weight_per_unit, 'gross_weight_lbs': dta.gross_weight, 'description': dta.description, 'sale_pallets': dta.purchase_pallets, 'boxes_pallet_for_purchase': dta.purchase_pallets, 'qty': dta.qty,'box': dta.qty, 'uom': dta.uom, 'price': dta.price, 'rate': dta.price, 'rate_lbs': dta.rate_lbs, 'box_unit_rate': dta.price, 'schedule_date': MasterPOItems.delivery_date,  'row_number' : dta.row_number,  'col_number' : dta.col_number};
			...
			console.log(TableArray1);
				 frappe.call({
					method:'erpnext.salesapi.saveMasterPoOrder',
					args: {
						po: MasterPOItems,
						ps: TableArray1
					},
