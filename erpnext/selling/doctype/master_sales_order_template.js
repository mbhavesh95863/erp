			...
			success: function( data ) {
				...
				if(data.data.master_sales_order_item.length > 0) 
				{
					...
					for (var i = 0; i < data.data.master_sales_order_item.length; i++) 
					{
						...
						var splcd = {'name': dta.name, 'qty': 0, 'received_qty': 0, 'price': 0, 'customer_name': dta.customer_name, 'customer': dta.customer , 'default_currency': dta.default_currency  , 'default_price_list': dta.default_price_list , 'mso_notes': mso_notes , 'po_no': po_no , 'address': address , 'address_title': address_title , 'col_number' : dta.col_number};
...
//console.log(TableArray);
			...
			for(var k=1; k < TableArray[v].length; k++)
			{
				...
				var dpl="STD";
				if(TableArray[1][k].default_price_list != "undefined" && TableArray[1][k].default_price_list && TableArray[1][k].default_price_list != null) { dpl = r.data[i]['default_price_list'];}
