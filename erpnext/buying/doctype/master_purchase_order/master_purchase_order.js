			...
			success: function( data ) {
				...
				if(data.data.master_purchase_order_item.length > 0) 
				{
					...
					for (var i = 0; i < data.data.master_purchase_order_item.length; i++) 
					{
						...
						var splcd = {'name': dta.name, 'qty': dta.qty, 'received_qty': dta.received_qty, 'price': dta.price, 'supplier_name': dta.supplier_name, 'supplier': dta.supplier , 'default_currency': dta.default_currency  , 'default_price_list': dta.default_price_list , 'col_number' : dta.col_number};
...
//console.log(TableArray);
			...
			for(var k=1; k < TableArray[v].length; k++)
			{
				...
				t1 = t1 + '<td data-row="1" data-col="'+ dataCol1 +'"><div class="autocomplete"><input type="hidden" class="isupplierid" name="isupplierid" value="'+ TableArray[1][k].supplier +'"><input type="hidden" class="default_currency" name="default_currency" value="'+ TableArray[1][k].default_currency +'"><input type="hidden" class="default_price_list" name="default_price_list" value="'+ TableArray[1][k].default_price_list +'"><input type="text" id="isupplier'+ dataCol1 +'" class="isupplier"  name="isupplier'+ dataCol1 +'" value="'+ TableArray[1][k].supplier +'"></div></td>';

table = table + '<td data-row="0" data-col="'+dataCol1 +'"><a href="javascript:;" class="delCol" onClick="myTable_DelCol('+ dataCol1 +')">'+show_x+'</a></td>';
	...
	tlCols = tlCols + '<td data-row="'+ (TableArray.length + 2) +'" data-col="'+ dataCol1 +'"><p id="qtotal" class="qtotal'+ dataCol1 +'">0</p><p id="stotal" class="stotal'+ dataCol1 +'">0.0</p></td>';		ttlcol  = colStart + k;
			...
			dataCol1 = dataCol1 + 1;			
					...
					titems = titems  + '<td data-row="'+dataRow +'" class="hidecol" data-col="'+dataCol1+'"><span class="itotal">0.0</span></td><td data-row="'+dataRow +'" data-col="'+ (dataCol1 + 1 )+'"><span class="avgitotal">0.0</span></td><td data-row="'+dataRow +'" data-col="'+ (dataCol1 + 2 )+'"><span class="qtytotal">0.0</span></td><td data-row="'+dataRow +'" data-col="'+ (dataCol1 + 3 )+'"><span class="pltitotal">0.0</span></td><td data-row="'+dataRow +'" data-col="'+ (dataCol1 + 4 )+'" class="hidecol"><span class="wgtitotal">0.0</span><span class="gwgtitotal">0.0</span></td></tr>';
		
		dataCol1 = 5;
		dataCol  = dataCol + 1;	
		dataRow = dataRow + 1;		
				}
				table  = table + '<td data-row="0" data-col="'+ttlcol+'" class="hidecol"></td><td data-row="0" data-col="'+ (ttlcol + 1) +'"></td><td data-row="0" data-col="'+ (ttlcol + 2) +'"></td><td data-row="0" data-col="'+ (ttlcol + 3) +'"></td><td data-row="0" data-col="'+ (ttlcol + 4) +'" class="hidecol"></td></tr>';
				
t1 = t1 + '<td data-row="1" class="hidecol" data-col="'+ttlcol +'"></td><td data-row="1" data-col="'+(ttlcol+ 1)+'"></td><td data-row="1" data-col="'+(ttlcol+ 2)+'"></td><td data-row="1" data-col="'+(ttlcol+ 3)+'"></td><td class="hidecol" data-row="1" data-col="'+(ttlcol+ 4)+'"></td></tr>'
;
t2 = t2 + '<td data-row="2" data-col="'+ttlcol +'" class="width100 hidecol">Actual Cost</td><td  class="width100" data-row="2" data-col="'+(ttlcol + 1)+'">Average Cost</td><td  class="width60" data-row="2" data-col="'+(ttlcol + 2)+'">Boxes</td><td class="width60" data-row="2" data-col="'+(ttlcol + 3)+'">Pallets</td><td class="width90 hidecol" data-row="2" data-col="'+(ttlcol + 4)+'">Ttl Weight</td></tr>';
				table = table  + t1;
				table = table + t2;
				table = table + titems;
table = table + '<tr class="item-row" data-row="'+dataRow+'"><td data-row="'+dataRow+'" data-col="0"></td><td data-row="'+dataRow+'" data-col="1"></td><td data-row="'+dataRow+'" data-col="2"></td><td data-row="'+dataRow+'" data-col="3"></td><td data-row="'+dataRow+'" data-col="4"></td>'+tlCols+'<td class="hidecol" data-row="'+dataRow+'" data-col="'+ ttlcol + '"><span class="itotal_total">0.0</span></td><td data-row="'+dataRow+'" data-col="'+ (ttlcol + 1 )+'"><span class="avgitotal_total">0.0</span></td><td data-row="'+dataRow+'" data-col="'+ (ttlcol + 2 )+'"><span class="qtytotal_total">0.0</span></td><td data-row="'+dataRow+'" data-col="'+ (ttlcol + 3 )+'"><span class="pltitotal_total">0.0</span></td><td data-row="'+dataRow+'" data-col="'+ (ttlcol + 4 )+'" class="hidecol"><span class="wgtitotal_total">0.0</span></td></tr>';
	...
	$wrapper.html('<div style="overflow-x:auto;"><table id="myTable"  border="1" class="display nowrap dataTable dtr-inline">'+table+'</table><script type="text/javascript">$("#myTable").on("click", ".isupplier", function() { var idd = $(this).attr("id"); autocompletesupplier(idd); }); $("#myTable").on("change", "input", function() { $(this).updatedata(); }); $("#myTable").on("click", ".sitems", function() { var row = $(this).parents(".item-row"); var idd = row.find(".sitems").attr("id"); autocomplete(idd); }); $("#myTable").find(".sitems").each( function(){ $(this).updatedata(); });</script></div>');
		...
		/* /Hide buttons coz we have separate modules/
		if(frm.doc.status == "Saved") {
		...
		{
			frm.add_custom_button(__("Draft Order"), function() {
				...
				$("#myTable").find(".sitems").each( function()
				{
					...
					if(row.find('.item_code').val() != '') 
					{
						...
						for(var i = 5; i < (colCount - 5); i++) 
						{
							...
							if($('#isupplier'+i).val() != '') {
							var default_price_list = sidcell.find('.default_price_list').val();
							col_number  = col_number + 1;
							var iqty = parseFloat(row.find('.iqty'+i).val());
							var rqty = parseFloat(row.find('.rqty'+i).val());
							var iprice = parseFloat(row.find('.iprice'+i).val());
							...
							var rate_lbs = 0;
							if(row.find('.weight').val() > 0) {
								var wt = parseFloat(row.find('.weight').val());
								rate_lbs = parseFloat(iprice) / wt;
							}
							...
							var listitm = {'name': iname, 'default_price_list': default_price_list, 'qty': iqty, 'price': iprice,'rate_lbs':rate_lbs, 'weight': row.find('.weight').val(), 'weight_per_unit': row.find('.weight').val(), 'gross_weight': row.find('.gross_weight').val(), 'uom': row.find('.uom').val(), 'description': row.find('.idesc').val(), 'purchase_pallets': row.find('.purchase_pallets').val(), 'item_code': row.find('.item_code').val(), 'item_name': row.find('.sitems').val(),'received_qty' : rqty, 'supplier_name': $('#isupplier'+i).val(), 'supplier': $('#isupplier'+i).val(), 'default_currency': sidcell.find('.default_currency').val(), 'row_number': vindex , 'col_number': col_number, 'total_boxes': total_boxes, 'total_pallet': total_pallet, 'total_net_weight': total_net_weight, 'total_gross_weight_lbs': total_gross_weight_lbs, 'total_weight_kg': total_weight_kg};
							MasterPOItems['master_purchase_order_item'].push(listitm);
			...
			var TableArray = [];
								...
								for (var i = 0; i < MasterPOItems.master_purchase_order_item.length; i++) 
								{
									...
									var itmcd1 = { 'supplier_name': dta.supplier_name, 'supplier': dta.supplier , 'default_price_list': dta.default_price_list,'arrival_date': frm.doc.arrival_date ,'shipping_date': frm.doc.shipping_date ,'transaction_date': frm.doc.transaction_date , 'cad': MasterPOItems.cad,  'usd': MasterPOItems.usd, 'conversion_rate': conversion_rate, 'plc_conversion_rate': plc_conversion_rate, 'order_type':frm.doc.order_type,  'row_number': dta.row_number, 'col_number':  dta.col_number, 'total_boxes': dta.total_boxes, 'total_pallet': dta.total_pallet, 'total_net_weight': dta.total_net_weight, 'total_gross_weight_lbs': dta.total_gross_weight_lbs, 'total_weight_kg': dta.total_weight_kg, items: [] };				
									var splcd = { 'item_name': dta.item_name ,  'item_code': dta.item_code, 'weight': dta.weight, 'weight_per_unit': dta.weight_per_unit, 'weight_lbs': dta.weight_per_unit,'gross_weight_lbs': dta.gross_weight,'rate_lbs': dta.rate_lbs, 'description': dta.description, 'purchase_pallets': dta.purchase_pallets, 'boxes_pallet_for_purchase': dta.purchase_pallets, 'qty': dta.qty, 'box': dta.qty, 'uom': dta.uom,'price': dta.price, 'box_unit_rate': dta.price, 'rate': dta.price, 'schedule_date': MasterPOItems.arrival_date,  'row_number' : dta.row_number,  'col_number' : dta.col_number};
			...
			//console.log(TableArray1); return false;
				 frappe.call({
					method:'erpnext.api2.saveMasterPoOrder',
					args: {
						po: MasterPOItems,
						ps: TableArray1
					},
						callback: function(r) {
						console.log(r.message);
						frappe.show_alert(r.message+ ' - submitted successfully ', 5);
						frappe.set_route("Form", "Master Purchase Order", r.message);
						//location.reload();
					}
				});  
